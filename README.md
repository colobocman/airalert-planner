...
Lines beginning with `#` are treated as comments and skipped, so a dataset can carry a provenance header. The bundled `data/sample_alerts.csv` is **synthetic** and is marked with exactly such a banner on its first line:

```text
# SYNTHETIC SAMPLE DATA - generated by scripts/generate_sample_data.py; NOT real air-raid alerts.
```

It is generated deterministically with a deliberately high-contrast pattern — near-silent daytime, busy nights, a steep evening ramp, and a wide spread between busy and quiet regions — so the risk scores are clearly demonstrative. For example, Kyiv reads ~0.01 at midday but climbs from ~0.26 to ~0.74 across 18:00–22:00, while quiet Lviv stays far below busy Kharkiv. Regenerate it with `python scripts/generate_sample_data.py`.
...
- The sample dataset is synthetic — generated by `scripts/generate_sample_data.py`, marked with a `# SYNTHETIC ...` banner on its first line, and built with a deliberately high-contrast pattern to demonstrate the method. It is not real alert data, so the validation lift reflects that built-in pattern, not forecasting accuracy.
